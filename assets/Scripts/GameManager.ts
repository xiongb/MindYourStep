import { _decorator, Component, Node,Prefab,instantiate,CCInteger, v3, LabelComponent } from "cc";
...
export class GameManager extends Component {
    ...
    generateRoad(){
        ...
        for (let j = 0; j < this._road.length; j++) {
            const element = this.spawnBlockByType(this._road[j]);
            if(element){
                this.node.addChild(element);
                element.setPosition(j,-1.5,0)
            }
        }
    }
